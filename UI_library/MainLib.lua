...
function Library:SetOption(id, value)
    if self.Options[id] then
        self.Options[id]:SetValue(value)
    end
end
        ...
        if tabIcon and tabIcon ~= "" then
            ...
            function tabMethods:AddToggle(id, options)
                ...
                toggleBackground.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        enabled = not enabled
                        Library.Options[id]:SetValue(enabled)
                    end
                end)
            ...
            function tabMethods:AddDropdown(id, options)
                ...
                -- Add corner radius to list frame
...
local ghostEspToggle = tab:AddToggle("GhostEspToggle", {
    Title = "Ghost ESP",
    Default = false,
...
ghostEspToggle:OnChanged(function(value)
    print("Toggle changed:", value) -- This will now correctly print the toggle state
    local ghost = workspace.Ghost

    if value then -- Using value directly from the callback
