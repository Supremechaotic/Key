...
function Library:CreateWindow(options)
    ...
    titleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            -- Check if we clicked directly on the title bar and not its children
            local mousePos = game:GetService("UserInputService"):GetMouseLocation()
            local titleBarPos = titleBar.AbsolutePosition
            local titleBarSize = titleBar.AbsoluteSize
            
            -- Get the element under the mouse
            local elementsAtMouse = game:GetService("CoreGui"):GetGuiObjectsAtPosition(mousePos.X, mousePos.Y)
            
            -- Only start dragging if we clicked directly on the title bar
            if elementsAtMouse[1] == titleBar then
                dragging = true
                ...
                for i, value in ipairs(values) do
                    ...
                    item.MouseButton1Click:Connect(function()
                        if multi then
                            ...
                            selected = value
                            updateText()
                            wait() -- Add a small delay before closing
                            toggleDropdown()
                        end
