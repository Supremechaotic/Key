        ...
        if tabIcon and tabIcon ~= "" then
            ...
            function tabMethods:AddDropdown(id, options)
                ...
                local dropdownList = CreateInstance("ScrollingFrame", {
                    Name = "List",
                    Parent = Library.WindowInstance.ScreenGui, -- Parent to ScreenGui instead of main window
                    BackgroundColor3 = Library.CurrentTheme.PrimaryElementColor,
                    BorderSizePixel = 0,
                    Position = UDim2.new(0, 0, 0, 0), -- Will be updated when opened
                ...
                game:GetService("UserInputService").InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and open then
                        ...
                        -- Check if click is outside both the dropdown list and the dropdown button
                        if not (
                            (mousePos.X >= dropdownPos.X and mousePos.X <= dropdownPos.X + dropdownSize.X and
                             mousePos.Y >= dropdownPos.Y and mousePos.Y <= dropdownPos.Y + dropdownSize.Y) or
