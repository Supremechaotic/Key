...
Library.Themes = {
    ...
    Light = {
        ...
        PrimaryTextColor = Color3.fromRGB(50, 50, 50),
        SecondaryTextColor = Color3.fromRGB(100, 100, 100),
        UIStrokeColor = Color3.fromRGB(200, 200, 200),
        PrimaryElementColor = Color3.fromRGB(220, 220, 220),
        SecondaryElementColor = Color3.fromRGB(210, 210, 210),
        OtherElementColor = Color3.fromRGB(200, 200, 200),
        ScrollBarColor = Color3.fromRGB(180, 180, 180),
        PromptColor = Color3.fromRGB(200, 200, 200),
        NotificationColor = Color3.fromRGB(230, 230, 230),
...
function Library:CreateWindow(options)
    ...
    -- Function to check if the clicked element is interactive
    local function isInteractiveElement(element)
        if not element then return false end
        
        -- Check if element is a button or other interactive element
        if element:IsA("TextButton") or element:IsA("TextBox") then
            return true
        end
        
        -- Check if element is part of a slider, dropdown, or other interactive component
        local parent = element.Parent
        while parent do
            if parent:IsA("TextButton") or parent:IsA("TextBox") or
               parent.Name:match("Slider_") or parent.Name:match("Dropdown_") or
               parent.Name:match("Colorpicker_") or parent.Name:match("Keybind_") or
               parent.Name:match("Input_") then
                return true
            end
    ...
    -- Handle input at the ScreenGui level
    screenGui.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            -- Check if we clicked on an interactive element
            local clickedElement = game:GetService("UserInputService"):GetMouseTarget()
            if not isInteractiveElement(clickedElement) then
                dragging = true
    ...
    function window:AddTab(options)
        ...
        local tabButton = CreateInstance("TextButton", {
            ...
            Position = UDim2.new(0, 0, 0, (tabIndex - 1) * 37),
            Font = Enum.Font.Gotham,
            Text = "",  -- Empty text, we'll add a separate TextLabel
        ...
        if tabIcon and tabIcon ~= "" then
            ...
            local iconImage = CreateInstance("ImageLabel", {
                ...
                Parent = tabButton,
                BackgroundTransparency = 1,
                Position = UDim2.new(0, 10, 0.5, -8),  -- Center vertically
            ...
            local textLabel = CreateInstance("TextLabel", {
                ...
                Parent = tabButton,
                BackgroundTransparency = 1,
                Position = UDim2.new(0, 36, 0, 0),  -- Position after the icon with padding
            ...
            tabButton.Text = "    " .. tabTitle  -- Add some padding
            ...
            function tabMethods:AddDropdown(id, options)
                ...
                local dropdownList = CreateInstance("Frame", {
                    Name = "List",
                    Parent = tabContent, -- Changed to tabContent to avoid clipping issues
                    BackgroundColor3 = Library.CurrentTheme.SecondaryElementColor,
                    BorderSizePixel = 0,
                    Position = UDim2.fromOffset(
                        dropdownContainer.AbsolutePosition.X + 10, 
                        dropdownContainer.AbsolutePosition.Y + (description ~= "" and 80 or 60)
                    ),
                ...
                for i, value in ipairs(values) do
                    ...
                    if multi then
                        ...
                        -- Item click handler for single select
                        item.MouseButton1Click:Connect(function()
                            selected = value
                            updateText()
                            toggleDropdown()
