...
-- Import Lucide Icons
local LucideIcons = local LucideIcons = loadstring(game:HttpGet("https://raw.githubusercontent.com/Supremechaotic/Key/refs/heads/main/UI_library/LucideIcons.lua"))()
...
function InterfaceManager:BuildInterfaceSection(tab)
    ...
    -- Add icon selector
    local availableIcons = LucideIcons:GetAvailableIcons()
    local iconDropdown = tab:AddDropdown("ThemeManager_IconDropdown", {
        Title = "Tab Icon",
        Description = "Select an icon for the current tab",
        Values = availableIcons,
        Default = "Settings",
        Callback = function(value)
            -- Update the current tab's icon
            local currentTab = self.Library.Tabs[self.Library.ActiveTab]
            if currentTab and currentTab.Button then
                local iconImage = LucideIcons:GetIcon(value)
                if iconImage then
                    -- Remove old icon if it exists
                    local oldIcon = currentTab.Button:FindFirstChild("Icon")
                    if oldIcon then
                        oldIcon:Destroy()
                    end
                    ...
                    -- Add new icon
                    iconImage.Name = "Icon"
                    iconImage.Parent = currentTab.Button
                    iconImage.Position = UDim2.new(0, 16, 0.5, -8)
                    iconImage.Size = UDim2.new(0, 16, 0, 16)
                    iconImage.ImageColor3 = currentTab.Index == self.Library.ActiveTab and
