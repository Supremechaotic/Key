...
function InterfaceManager:ApplyTheme(theme)
    ...
    -- Update title bar and controls
    if self.Library.WindowInstance and self.Library.WindowInstance.TitleBar then
        local titleBar = self.Library.WindowInstance.TitleBar
        titleBar.BackgroundColor3 = themeData.SidebarColor
        
        -- Update window title text
        local titleText = titleBar:FindFirstChild("Title")
        if titleText then
            titleText.TextColor3 = themeData.PrimaryTextColor
        end
        
        -- Update minimize button
        local minimizeBtn = titleBar:FindFirstChild("MinimizeBtn")
        if minimizeBtn then
            minimizeBtn.ImageColor3 = themeData.PrimaryTextColor
            if minimizeBtn:FindFirstChild("HoverFrame") then
                minimizeBtn.HoverFrame.BackgroundColor3 = themeData.AccentColor
            end
        end
        ...
        if closeBtn then
            closeBtn.ImageColor3 = themeData.PrimaryTextColor
            if closeBtn:FindFirstChild("HoverFrame") then
                closeBtn.HoverFrame.BackgroundColor3 = themeData.AccentColor
            end
        end
    end
    ...
    for _, option in pairs(self.Library.Options) do
        if option.Instance then
            ...
            elseif option.Type == "Button" then
                ...
                local buttonText = option.Instance:FindFirstChild("Title")
                if buttonText then
                    buttonText.TextColor3 = themeData.PrimaryTextColor
                end
                
                -- Update hover frame if exists
